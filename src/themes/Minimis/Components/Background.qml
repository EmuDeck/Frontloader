...
Item {
    ...
    ImageCrossfade {
        ...
        anchors.fill: parent
        opacity: showBackgroundImage ? api.memory.get('settings.general.backgroundOpacity') : 0
        layer.enabled: true
        ...
        layer.effect: OpacityMask {
            layer.enabled: api.memory.get('settings.general.backgroundBlurEnabled')
            ...
            layer.effect: FastBlur {
                radius: api.memory.get('settings.general.backgroundBlurAmount')
            }
...
seed) {
return
fract(sin(dot(seed, vec2(12.9898, 78.233)))
* 43758.5453);
}
