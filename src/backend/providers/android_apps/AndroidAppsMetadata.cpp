...
namespace providers
{
	namespace android
	{
		...
		MetadataHelper::MetadataHelper(QString log_tag)
				: m_log_tag(std::move(log_tag)), m_json_cache_dir(QStringLiteral("androidapps")),
				  rx_meta_itemprops(QStringLiteral(R""(<meta itemprop="(.+?)" content="(.+?)")""),
				                    QRegularExpression::DotMatchesEverythingOption),
				  rx_background(QStringLiteral(R""(<meta property="og:image" content="(.+?)")"")), rx_developer(
						QStringLiteral(
								R""(<a href="/store\/apps\/dev(eloper)?\?id=.+?" class=".*?">([^<]+)<\/a><\/span>)"")),
				...
				SearchContext &sctx) const
		{
			...
			sctx.schedule_download(url, [this, app_package, game_ptr](QNetworkReply* const reply)
			{
				...
				if (success)
					providers::cache_json(m_log_tag, m_json_cache_dir, app_package,
