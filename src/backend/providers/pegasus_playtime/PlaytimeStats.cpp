...
namespace providers
{
	namespace playtime
	{
		...
		PlaytimeStats::PlaytimeStats(QString db_path, QObject* parent)
				: Provider(QLatin1String("pegasus_playtime"), QStringLiteral("Playtime"),
			...
			QtConcurrent::run([this]
			                  {
				                  ...
				                  while (!m_active_tasks.empty())
				                  {
					                  ...
					                  if (!channel.open())
					                  {
						                  Log::warning(display_name(),
						                               LOGMSG("Could not open or create `%1`, play time will not be saved")
								                               .arg(m_db_path));
						                  break;
					                  }
					                  ...
					                  for (const QueueEntry &entry: m_active_tasks)
					                  {
						                  ...
						                  if (path_id >= 0)
							                  save_play_entry(display_name(), path_id, entry.launch_time,
							                                  entry.duration);
					                  }
