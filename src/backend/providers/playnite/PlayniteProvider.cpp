	...
	namespace playnite
	{
		...
		namespace
		{
					...
					const QDir &playnite_dir)
			{
				...
				{
					const QString cover_path = QStringLiteral("%1/library/files/%2").arg(playnite_dir_path,
				...
				const QString extra_metadata_path = QStringLiteral("%1/ExtraMetadata/games/%2").arg(playnite_dir_path,
				                                                                                    game_info.id);
		...
		bool PlayniteProvider::validate_game(const PlayniteGame &game, const PlayniteComponents &components) const
		{
			...
			const PlayniteGameLaunchInfo::LaunchType launch_type = PlayniteLaunchHelper::get_launch_type_enum(
					game.play_action.type);
			if (launch_type == PlayniteGameLaunchInfo::LaunchType::INSTALLED)
			{
				...
				if (game.install_directory.isEmpty())
				{
					Log::info(display_name(),
					          LOGMSG("No install directory found for game %1, skipping...").arg(game.name));
					return false;
				}
			}
			else if (launch_type == PlayniteGameLaunchInfo::LaunchType::STEAM)
			{
				...
				{
					Log::info(display_name(),
					          LOGMSG("Unsupported source %1 for game %2, skipping...").arg(source, game.name));
					return false;
				}
			...
			else if (launch_type == PlayniteGameLaunchInfo::LaunchType::EMULATOR)
			{
				...
				{
					Log::info(display_name(),
					          LOGMSG("No game image path found for game %1, skipping...").arg(game.name));
					return false;
				}
				...
				{
					Log::info(display_name(),
					          LOGMSG("No emulator profile found for game %1, skipping...").arg(game.name));
					return false;
				}
			...
			{
				Log::info(display_name(), LOGMSG("Unknown play action type %1 for game %2, skipping...").arg(
						QString::number(game.play_action.type), game.name));
				return false;
			}
			return true;
		}
