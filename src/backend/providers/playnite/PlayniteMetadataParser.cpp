...
namespace providers
{
	namespace playnite
	{
		...
		PlayniteMetadataParser::PlayniteMetadataParser(QString log_tag, const QDir &playnite_dir)
				: m_log_tag(std::move(log_tag)), m_playnite_dir(playnite_dir),
		...
		HashMap<QString, PlayniteEmulator> PlayniteMetadataParser::parse_emulator_metadata() const
		{
			...
			QDirIterator dir_it(m_playnite_dir.filePath(QStringLiteral("library/emulators/")), m_json_ext_list,
			                    m_dir_filters);
		...
		std::vector<PlayniteGame> PlayniteMetadataParser::parse_game_metadata() const
		{
			...
			QDirIterator dir_it(m_playnite_dir.filePath(QStringLiteral("library/games/")), m_json_ext_list,
			                    m_dir_filters);
			while (dir_it.hasNext())
			{
				const QString file_path = dir_it.next();
