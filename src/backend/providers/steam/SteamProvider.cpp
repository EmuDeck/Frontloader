...
namespace
{
	...
	std::vector<QString> find_steam_installdirs(const QString &log_tag, const QString &steam_datadir)
	{
		...
		VEC_REMOVE_IF(installdirs, [](const QString &path)
		{ return !QFileInfo::exists(path); });
		return installdirs;
	}

	void fill_metadata_from_cache(HashMap<QString, model::Game*> &appid_game_map,
...
namespace providers
{
	namespace steam
	{
		...
		Provider &SteamProvider::run(SearchContext &sctx)
		{
			...
			{
				HashMap<QString, model::Game*> local_games = gamehelper.find_in(steam_call, installdir, collection,
				                                                                sctx);
