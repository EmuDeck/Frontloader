...
namespace providers
{
	namespace gog
	{
		...
		Metadata::Metadata(QString log_tag)
				: m_log_tag(std::move(log_tag)), m_json_cache_dir(QStringLiteral("gog")),
				  m_json_api_suffix(QStringLiteral("_api")), m_json_embed_suffix(QStringLiteral("_embed"))
		{}
		...
		{
			const QString api_url_str = QStringLiteral(
					"https://api.gog.com/products/%1?expand=description,screenshots,videos").arg(gogid);
			const QString embed_url_str = QStringLiteral(
					"https://embed.gog.com/games/ajax/filtered?mediaType=game&search=%1").arg(gogid);
			...
			for (const auto &triplet: requests)
			{
				...
				sctx.schedule_download(std::get<0>(triplet),
				                       [log_tag, json_cache_dir, gogid, game_ptr, json_suffix, json_callback](
