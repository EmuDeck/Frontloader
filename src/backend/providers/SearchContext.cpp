...
namespace providers
{
	...
	SearchContext::SearchContext(QStringList game_dirs, QObject* parent)
			: QObject(parent), m_root_game_dirs(std::move(game_dirs)), m_netman(nullptr), m_pending_downloads(0)
	{}
	...
	void SearchContext::finalize_cleanup_games()
	{
		...
		for (auto &pair: m_collection_games)
		{
			...
			VEC_REMOVE_IF(game_list, [this](model::Game* const ptr)
			{ return m_game_entries.count(ptr) == 0; });
		}
