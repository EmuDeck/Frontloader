...
namespace providers
{
	namespace launchbox
	{
		...
		GamelistXml::GamelistXml(QString log_tag, QDir lb_root)
				: m_log_tag(std::move(log_tag)), m_lb_root(std::move(lb_root)), m_game_keys{
			...
			if (emu_id_it != fields.cend())
			{
				...
				if (emu_it == emulators.cend())
				{
					log_xml_warning(xml_path, xml_linenum,
					                LOGMSG("Game refers to a missing or invalid emulator with id `%1`, entry ignored").arg(
							                emu_id_it->second));
					return false;
				}
				...
				const HashMap<AppField, QString> &fields) const
		{
			...
			{
				log_xml_warning(xml_path, xml_linenum,
				                LOGMSG("Additional application has no GameID field, entry ignored"));
				return false;
			}
			...
			{
				log_xml_warning(xml_path, xml_linenum,
				                LOGMSG("Additional application file `%1` doesn't seem to exist, entry ignored")
		...
		{
			const QString xml_rel_path = QStringLiteral("Data/Platforms/%1.xml").arg(
					platform.name); // TODO: Qt 5.14+ QLatin1String
			...
			while (xml.readNextStartElement())
			{
				if (xml.name() == QLatin1String("Game"))
				{
					...
					if (source == QLatin1String("Steam"))
					{
						...
						if (!match.hasMatch())
						{
							log_xml_warning(xml_path, linenum,
							                LOGMSG("Game was expected to be a Steam game, but its path field seems to be incorrect"));
							continue;
						}
					...
					else
					{
						...
						{
							log_xml_warning(xml_path, linenum,
							                LOGMSG("Game file `%1` doesn't seem to exist, entry ignored").arg(
									                ::pretty_path(game_path)));
							continue;
						}
			...
			for (const HashMap<AppField, QString> &fields: addiapps)
			{
				...
				if (it == gameid_map.cend())
				{
					...
					Log::warning(m_log_tag,
					             LOGMSG("In `%1` additional application entry `%2` refers to missing or invalid game `%3`, entry ignored")
