...
namespace providers
{
	namespace launchbox
	{
		...
		EmulatorsXml::EmulatorsXml(QString log_tag, QDir lb_root)
				: m_log_tag(std::move(log_tag)), m_lb_root(std::move(lb_root)), m_emulator_keys{
				...
				const HashMap<PlatformField, QString> &fields) const
		{
			...
			if (it_id == fields.cend())
			{
				log_xml_warning(xml_path, xml_linenum,
				                LOGMSG("Emulator platform has no emulator ID field, entry ignored"));
				return false;
			}
				...
				const HashMap<EmulatorField, QString> &fields) const
		{
			...
			{
				log_xml_warning(xml_path, xml_linenum,
				                LOGMSG("Emulator executable `%1` doesn't seem to exist, entry ignored").arg(
						                ::pretty_path(it_path->second)));
				return false;
			}
				...
				if (it == emulators.cend())
				{
					for (const EmulatorPlatform &platform: entry.second)
					{
						Log::warning(m_log_tag,
						             LOGMSG("In `%1` emulator platform `%2` refers to a missing or invalid emulator entry with id `%3`, entry ignored")
