...
namespace
{
	...
	bool read_datfile_intro(const QString &log_tag, const QString &pretty_path, QXmlStreamReader &xml)
	{
		...
		{
			Log::warning(log_tag,
			             LOGMSG("`%1` seems to be a valid XML file, but doesn't have a DOCTYPE declaration, ignored").arg(
					             pretty_path));
			return false;
		}
		...
		{
			Log::warning(log_tag,
			             LOGMSG("`%1` seems to be a Logiqx file, but doesn't start with a `datafile` root element").arg(
					             pretty_path));
			return false;
		}
		if (xml.hasError())
			...
			providers::SearchContext &sctx)
	{
		...
		{
			Log::warning(log_tag,
			             LOGMSG("The `game` element in `%1` at line %2 has an empty or missing `name` attribute, entry ignored")
		...
		while (xml.readNextStartElement())
		{
			...
			if (xml.name() == QLatin1String("rom"))
			{
				...
				{
					Log::warning(log_tag,
					             LOGMSG("The `rom` element in `%1` at line %2 has an empty or missing `name` attribute, ignored")
							             .arg(pretty_path, QString::number(xml.lineNumber())));
					continue;
				}
				...
				{
					Log::warning(log_tag,
					             LOGMSG("The `rom` element in `%1` at line %2 refers to file `%3`, which doesn't seem to exist")
				...
				{
					Log::warning(log_tag,
					             LOGMSG("The `rom` element in `%1` at line %2 seems to be a duplicate entry, ignored")
							             .arg(pretty_path, QString::number(xml.lineNumber())));
					continue;
				}
		...
		{
			Log::warning(log_tag,
			             LOGMSG("The `game` element in `%1` at line %2 has no valid `rom` fields, game ignored")
					             .arg(pretty_path, QString::number(game_start_linenum)));
			return;
		}
