	...
	m_future = QtConcurrent::run([this]
	                             {
		                             ...
		                             for (size_t i = 0; i < providers.size(); i++)
		                             {
			                             ...
			                             const bool has_progress = !(provider.flags() &
			                                                         providers::PROVIDER_FLAG_HIDE_PROGRESS);
			                             if (has_progress)
		                             ...
		                             if (sctx.has_pending_downloads())
		                             {
			                             ...
			                             connect(&sctx, &providers::SearchContext::downloadCompleted,
			                                     &loop, [&loop, &sctx]
			                                     { if (!sctx.has_pending_downloads()) loop.quit(); });
			                             loop.exec();

			                             Log::info(LOGMSG("Waiting for online sources took %1ms").arg(
					                             network_timer.elapsed()));
		                             }
		                             ...
		                             Log::info(LOGMSG("Game list post-processing took %1ms").arg(
				                             finalize_timer.elapsed()));
		                             emit scanFinished();
	                             });
}
