...
namespace providers
{
	...
	public:
		...
		const QStringList &root_game_dirs() const
		{ return m_root_game_dirs; }

		const QStringList &pegasus_game_dirs() const
		{ return m_pegasus_game_dirs; }
		...
		const HashMap<QString, model::GameFile*> &current_filepath_to_entry_map() const
		{ return m_filepath_to_gamefile; }
