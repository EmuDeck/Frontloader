...
namespace model
{
	...
	QVariant GamepadListModel::data(const QModelIndex &index, int role) const
	{
		...
		{
			case Roles::Self:
				return QVariant::fromValue(gamepad_ptr);
			case Roles::Name:
				return gamepad.name();
			case Roles::DeviceId:
	...
	{
		const auto pred = [device_id](const model::Gamepad* const gp)
		{ return gp->deviceId() == device_id; };
