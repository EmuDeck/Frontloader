...
namespace model
{
	ThemeEntry::ThemeEntry(QString root_dir, QString root_qml, QString name,
			...
			: root_dir(std::move(root_dir)), root_qml(std::move(root_qml)), name(std::move(name)),
			  author(std::move(author)), version(std::move(version)), summary(std::move(summary)),
	...
	Themes::Themes(QObject* parent)
			: QAbstractListModel(parent), m_role_names({
			                                           ...
			                                           }), m_themes(find_available_themes()), m_current_idx(0)
	{}
