...
namespace model
{
	...
	QVariant GameListModel::data(const QModelIndex &index, int role) const
	{
		...
		{
			case Roles::Self:
				return QVariant::fromValue(game_ptr);
			case Roles::Title:
				return game.title();
			case Roles::SortBy:
				return game.sortBy();
			...
			case Roles::ReleaseMonth:
				return game.releaseMonth();
			case Roles::ReleaseDay:
				return game.releaseDay();
			...
			case Roles::Extra:
				return game.extraMap();
			case Roles::Developer:
				return game.developerStr();
			case Roles::DeveloperList:
				return game.developerListConst();
			case Roles::Publisher:
				return game.publisherStr();
			case Roles::PublisherList:
				return game.publisherListConst();
			case Roles::Genre:
				return game.genreStr();
			case Roles::GenreList:
				return game.genreListConst();
			case Roles::Tag:
				return game.tagStr();
			case Roles::TagList:
				return game.tagListConst();
			case Roles::Assets:
				return QVariant::fromValue(game.assetsPtr());
			case Roles::Files:
				return QVariant::fromValue(game.filesModel());
			case Roles::Collections:
				return QVariant::fromValue(game.collectionsModel());
			default:
				return {};
		}
	}
	...
	void GameListModel::connectEntry(model::Game* const game)
	{
		connect(game, &model::Game::favoriteChanged,
		        this, [this]()
		        { onGamePropertyChanged({Roles::Favorite}); });
		connect(game, &model::Game::playStatsChanged,
		        this, [this]()
		        { onGamePropertyChanged({Roles::PlayCount, Roles::PlayTime, Roles::LastPlayed}); });
	}
