...
FolderListEntry::FolderListEntry(QString name, bool is_dir)
		: name(std::move(name)), is_dir(is_dir)
{}


FolderListModel::FolderListModel(QObject* parent)
		: QAbstractListModel(parent), m_dir(startup_dir()), m_drives_cache(drives()), m_role_names({
				                                                                                           {EntryName,  "name"},
				                                                                                           {EntryIsDir, "isDir"},
		                                                                                           })
{
	...
	else
	{
		...
		erase_if(filist, [this](const QFileInfo &fi)
		{
			...
			const bool has_matching_ext = std::any_of(
					m_extensions.cbegin(),
					m_extensions.cend(),
					[&fi](const QString &suffix)
					{ return fi.fileName().endsWith(suffix); });
			if (has_matching_ext)
				return false;
